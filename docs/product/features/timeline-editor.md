...
1. **Wrong durations are hard to find in a table.** `docs/risks.md` **R4** is that a `SystemInferred` end silently mis-bills. The History View renders `end_determination` as the word `inferred` in italic (`+page.svelte:760-764`, styled `:938-945`) — honest, but it does not tell you *which* inferred end is wrong.
...
**2. A shared *view* range across the timeline and the History View. Export keeps its own range.** *(Rewritten twice. See Alternative L, where this is argued.)* There is no dashboard range today — `resolveRange` serves `doExport` alone (`+page.svelte:366, 404`), and the History View is unfiltered (`:489`). So this decision creates a **view** range, and deliberately does not extend it to export.
