...
2. **A shipped error message instructs the user to perform an action that does not exist.** `StackError::BlockReferencedByOpenFrame` reads: *"cannot be reshaped or deleted while an unresolved interruption still refers to it — resume or **dismiss** that interruption first."* There is no dismiss. `TransitionPayload` has fourteen variants and none of them dismisses a frame; no command exposes one. The only way to clear a frame today is to return to it. This is a textbook **R11** instance living in shipped code rather than in a document, and it is reachable from the Timeline Editor's restricted-block tier and from Delete.
...
- **The dismissal transition does not exist and must be built.** `TransitionPayload` has no dismissal variant and `commands.rs` exposes no command. It must resolve the named frame's block to `Skipped` via the same `resolve_paused` path the returns use, remove that frame, and leave `active` untouched. It is a domain change, so it belongs to the state machine and its tests — not to the panel.
...
- **`StackError::BlockReferencedByOpenFrame`'s message becomes true when this ships.** Today it advises an action that does not exist. Either the transition ships with this feature and the text stands, or the text must be corrected — it may not keep pointing at a capability nobody built. Tracked below as an acceptance criterion rather than left to notice.
