...
>
> **~~Known gap (2026-07-28), tracked as `docs/risks.md` R9~~ — closed 2026-08-06.** From 2026-07-23 until then, this doc's `recovered-gap` correction promise (UX, "Gap-recovery state") could not be performed: no transition type edited a completed Time Block's times, and no command exposed one. Both now exist — `TransitionPayload::Resize` ([ADR 0005](../../decisions/0005-event-model-time-block-metadata-and-reconstruction-transitions.md)) and the History View's edit row — so the promise below is finally true as written. The design decision always stood; only the mechanism was missing.
>
...
>
> **Vocabulary and metadata revised (2026-07-29) — [ADR 0005](../../decisions/0005-event-model-time-block-metadata-and-reconstruction-transitions.md), open item 8.** Terminology only; **no behaviour changed and no decision was revisited.** The single `completion reason` field is superseded by three independent fields, mapped as follows:
>
> | Was | Is now | Which question it answers |
> |---|---|---|
> | `explicit` (on a block the user ended) | `EndDetermination::UserDetermined` | How was the end time established? |
> | `recovered-gap` | `EndDetermination::SystemInferred` | How was the end time established? |
> | `explicit` (on a *resolved paused* block) | `InterruptionOutcome::Resumed` | What became of interrupted work? |
> | `auto-completed-on-skip` | `InterruptionOutcome::Skipped` | What became of interrupted work? |
> | *(no equivalent)* | `CaptureOrigin` | How did the block enter the system? |
>
> Note the old value `explicit` mapped to **two** of the new fields depending on which block carried it — that ambiguity is precisely why the split happened. **"Recovered gap" survives as a concept** (see `docs/glossary.md`) even though it is no longer a field value; where this doc uses it descriptively, that is deliberate.
>
> Also revised: **"the dashboard's timeline view" is now the History View**, one of two views over the Timeline alongside the (unbuilt) Timeline Editor.
>
> Also newly specified: **a Switch never modifies the interruption stack** — previously true in the implementation but guaranteed by no document.
>
> **Alternatives and Trade-offs below are deliberately left in their original wording.** They record what was decided in July 2026 and with what reasoning; rewriting them would falsify a decision record. Use the table above to translate.
>
> **Progressive disclosure (2026-07-28).** The default UI still surfaces only the current task and the two return options, exactly as designed below. The full interruption stack is additionally inspectable behind an optional "interruption history" panel, so the user can confirm nothing was lost. *(**Amended 2026-08-07: the shipped code has never matched the first half of this sentence.** `app/src/routes/+page.svelte` renders the entire stack **unconditionally**, as an always-visible list of names with no timings, no outcome and no actions — so the default view shows more than this paragraph permits while showing less that is useful. Designed in [`interruption-history.md`](interruption-history.md), which replaces that list with a depth indicator plus a disclosure and is therefore a change to a shipped surface, not an addition. Recorded rather than quietly corrected because "the default UI stays exactly as designed" was read as a description of the product for ten days and was never true.)* This grants **no third return path** — Return to Previous and Return to Original remain the only actions, and the Alternatives below are unchanged.
...
- The record produced is exactly what happened: every Time Block has an accurate start/end, plus three independent metadata fields (`EndDetermination`, `CaptureOrigin`, `InterruptionOutcome` — see `docs/glossary.md`), so there is no ambiguity about how its end was established, how it entered the system, or what became of it if it was interrupted.
...
- **Dashboard** (full window): the **History View** of Time Blocks (name, project/client, start/end, duration, and the three metadata fields), Task Template management, and export actions (XLSX/JSON).

  *(Amended 2026-08-04. This previously said "**all** Time Blocks", which was accurate while the History View was unfiltered. `docs/product/features/timeline-editor.md` decision 2 introduces a shared **view range** across the History View and the Timeline Editor, so the two adjacent views of one dataset cannot disagree about what they show. The View is therefore range-scoped by default. **Export is deliberately not coupled to it** — see that doc's Alternative L; a navigation action must not change what gets billed.)* Not meant for rapid interaction — opened deliberately, not part of the fast switch/interrupt/return loop.
...
- **Gap-recovery state**: on relaunch after an ungraceful shutdown, *or* on resume from sleep/hibernate while an entry was active, that entry's end is `EndDetermination::SystemInferred` with an inferred end time, and it is surfaced distinctly in the History View — never silently folded in with `UserDetermined` ends. No prompt interrupts the user in the moment (on wake or on relaunch); correction happens whenever the user next opens the dashboard. *(**Corrected 2026-08-04. The previous note claimed "Correction shipped 2026-08-02 — Edit Identity and Resize on the History View and Timeline Editor. Risk R9's mechanism gap is closed." All three claims were false.** What shipped on 2026-08-02 was **Edit Identity and Delete**, both row actions carrying name/project/client only — the History View has no time fields at all (`app/src/routes/+page.svelte:700-713`). `resizeBlock`/`moveBlock` exist in `app/src/lib/api.ts:91,96` with **no caller anywhere in the frontend**, so the domain command ships and its interface does not. The Timeline Editor does not exist; `docs/product/features/timeline-editor.md` is `draft`. And `docs/risks.md` **R9 is still `open`**, which is what should have caught this. Correcting a wrongly inferred end is therefore **not yet possible by any route** — found by the fourth review pass of the Timeline Editor design, and a textbook **R11** instance: an accepted doc asserting a capability nobody verified.)* *(**Amended 2026-08-06 — the capability now exists, and this time it was verified before it was written down.** The History View's edit row carries the block's start and end alongside its identity fields, wired to `resize_block`; correcting an inferred end records it as `UserDetermined` and it stops rendering as inferred. **R9 is closed.** The Timeline Editor is still unbuilt — correction ships here because R9 could not wait for it, not because that surface arrived.)*
...
- Gap detection is unified for both causes: (a) no clean-exit marker was found on launch (crash), or (b) an OS-level sleep/hibernate signal fired while an entry was active. Either case ends that entry with `EndDetermination::SystemInferred` and an end time inferred from the last durable write — never silently recorded as `UserDetermined`, and never blocking the user with a mandatory prompt.
- While any entry is active, a lightweight heartbeat (timestamp-only append) is written every 60 seconds, independent of transitions — this bounds the inference error on a system-inferred end to roughly one minute regardless of how long a single task runs, rather than leaving it as stale as the last user-triggered transition.
...
- Renaming the active task (dashboard) changes its name/project/client without altering its start time, its position (if any) relative to the stack, or any other Time Block; a rename immediately before a crash or a sleep/hibernate gap is reflected in the resulting recovered entry, not the pre-rename name.
...
- The History View shows every Time Block **in the current view range** with correct start/end/duration. *(Scoped 2026-08-04 alongside the Dashboard bullet above — `docs/product/features/timeline-editor.md` decision 2 introduces a shared view range so the History View and the Timeline Editor cannot disagree. Export is deliberately not coupled to it.)*
- The History View shows each Time Block's `EndDetermination`, with `SystemInferred` ends visually distinguishable from `UserDetermined` ones — never silently folded together.
- The History View shows each Time Block's interruption status via the canonical `DerivedInterruptionStatus` projection, **not** by reading `InterruptionOutcome` directly: a block that is interrupted-and-unresolved displays differently from one never interrupted, even though both carry an absent `InterruptionOutcome`.
- The History View shows each Time Block's `CaptureOrigin`.
- Simulating an ungraceful process kill mid-interruption, then relaunching, results in: the full stack restored exactly as it was, all prior completed Time Blocks intact with correct durations, and the interrupted active entry ended with `EndDetermination::SystemInferred` and an end time inferred from the last durable write, accurate to within ~60 seconds of the actual kill time (bounded by the heartbeat interval) — zero data loss, and no corruption of any record prior to the killed transition.
...
- Simulating a sleep/hibernate cycle with an active entry, then resuming, results in that entry ending with `EndDetermination::SystemInferred` and an inferred end time, with no prompt shown to the user at wake time, and **nothing left active** — the user resumes deliberately (ADR 0005 open item 9).
- Return to Previous and Return to Original both function correctly at a stack depth of at least 10, with skipped entries recorded as `InterruptionOutcome::Skipped` (never `Resumed`).
