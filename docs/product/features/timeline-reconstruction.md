...
>
> **Accepted is not ready to implement.** Three things gated that, none of them design questions. **One is now closed:** risk **R14** — a `seq` consumed by an append that did not durably complete — was fixed in the writer on 2026-08-02, so `seq` uniqueness is now an invariant the writer enforces rather than one ADR 0004 assumed. **A second is also closed:** the compaction snapshot (#8) shipped 2026-08-02 carrying every block's `id`, so a block below the watermark stays a valid reconstruction target — assumption **A14**. *(A third — undo's identity question — was closed the same day: Delete is confirmed and has no undo, so nothing is ever re-created and ADR 0006's revisit trigger did not fire. See the Delete bullet under UX.)* **One remains:** the Timeline Editor (#14) owes Add/Move/Resize a surface. Edit Identity and Delete are not Editor-gated and can ship on the History View (#6) ahead of it.
>
...
>
> **Edit Identity and Delete are not Editor-gated.** They need a selected row, not direct manipulation, and both accepted docs that speak to this already scope the dependency that way: `docs/product/mvp.md` makes the Editor a hard prerequisite for **move and resize specifically** — naming two operations, not five — and `docs/glossary.md` defines the History View as being for "reading **and editing** work as a structured list." An earlier draft of this header gated all five on #14, which would have sequenced **R8**'s only mitigation (correcting an uncorrected `Anchor N` on a closed block) behind the visual redesign *and* the Editor for no reason either doc supports. The two operations that unblock **R8** and **R9** can ship on the surface that already exists. *(Both since have: **Edit Identity on 2026-08-02**, unblocking R8, and **Resize on 2026-08-06**, closing R9 — neither waiting on #14, exactly as this paragraph argued.)* *(Quotation removed 2026-08-04: `mvp.md` used to say move and resize are "meaningless against the tabular History View" and now says they are **unjudgeable** there — `timeline-editor.md`'s numeric fallback disproved the stronger claim. The reasoning above is unaffected, since it rests on the two/five split, not on that word.)*
>
...
*(Added 2026-08-02, when Edit Identity and Delete were implemented. Deliberately a subsection here rather than a History View feature document: this doc set is organised by **capability**, not by screen — reconstruction operations belong to this doc, presentation to `visual-redesign.md` — and a screen-shaped document would mostly restate accepted behaviour from both.)*

- **Edit Identity is initiated from the History View**, as a row action. It opens the same three fields `Rename` uses, on a block that has already finished.
- **Delete is initiated from the History View**, also as a row action.
- **Delete always requires confirmation**, and cancelling performs no transition at all — nothing is written to the log. MVP has no undo, so the confirmation is the only guard; the persona rule reserves confirmations for exactly this case.
- **The Timeline Editor (#14) remains responsible only for direct timeline manipulation** — Add, Move, Resize. Those need a spatial surface; these two need a selected row.

**The surface stays a thin adapter, and that is a constraint rather than an implementation note.** It collects input and calls a command; every rule — the block must exist, must not be the currently active one, must not have an unresolved interruption frame pointing at it — is enforced in the state machine, which replay also runs. The UI does not pre-empt those checks, and a rejected operation surfaces the domain's own error. Duplicating a rule in the UI is how the editor and replay start to disagree, and it is the same argument as "the editor clamps; the domain rejects" below. Enforced by test: every successful row action appends **exactly one** transition, and a rejected one appends none.
...
- **Delete must not let an `Anchor N` name be reissued.** `stack.rs`'s `next_default_name` takes the maximum `N` among *today's* blocks, so physically deleting today's highest-numbered auto-named block makes the next unnamed task reuse that name — two unrelated pieces of work under one name on one day, which export then groups into a single row. That is **R8** arriving through the delete path — in fact its inverse: R8 covers an un-renamed `Anchor N` *fragmenting* totals, this is two unrelated tasks *merging* under one. Found during the undo design pass; it is a property of Delete itself, not of undo. **Tracked as its own backlog item ([#19](https://github.com/DeLicious777/Anchor/issues/19))** rather than only as a note here, since the consequence is user-visible and lands in a billing export. Latent until Delete ships.
...
- **`EndDetermination` after an edit.** Any operation that sets a block's **end** makes it `UserDetermined` — Add, Resize of the end boundary, and Move (which sets both). Editing only the *start* leaves it untouched, because it says nothing about how the end was established.
  ...
  **The reverse direction has no reachable trigger, and that is worth stating rather than leaving as a rule nothing implements.** *(Implementation evidence, 2026-08-02, from building Add/Move/Resize.)* Verified against the code: exactly one site can ever write `SystemInferred` — `RecoverGap`, in `stack.rs` — and every reconstruction operation is a user action, so it writes `UserDetermined`. Nothing in this feature can move a block *back* to `SystemInferred`. The rule above is therefore correct and load-bearing in the forward direction, and inert in the reverse one.
  
  It was written during the undo design pass, where the reverse case *was* reachable: undoing a Resize would have restored an inferred end. **Undo was then not adopted** (Delete is confirmed, MVP has no undo), which removed the only path to it. Recorded rather than deleted, because the rule becomes live again the moment anything restores a previous end — deferred commit, a future undo, or an import that carries prior state. Whichever arrives must also carry the *original* determination, since the domain does not retain which end value was inferred once it has been replaced.
...
- Correcting a block's end **back** to the value Anchor originally inferred leaves its `EndDetermination` as `UserDetermined`. It does **not** revert to `SystemInferred`. *(**Corrected 2026-08-06 against the implementation, which this criterion has contradicted since the day it was written.** `stack.rs`'s Resize arm sets `UserDetermined` whenever `end_changed`, and has no path back to `SystemInferred` — only `RecoverGap` writes that value. Reverting would require remembering the originally inferred instant, which is state [ADR 0005](../../decisions/0005-event-model-time-block-metadata-and-reconstruction-transitions.md) does not carry, so this was never merely unimplemented — it was unbuildable without a new persisted field. The code is also the more honest of the two: on the second edit the user really did determine that instant, and a record claiming Anchor inferred it would be false. **What this criterion was protecting against is real and is separately guaranteed**: a **start-only** resize must not silently claim the user fixed an inferred end. That holds, and is tested — changing only the start leaves the determination untouched. Found by the fifth review pass of `timeline-editor.md`, while writing the verification step that would have exercised it.)*
