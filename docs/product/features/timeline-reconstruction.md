...
- **Delete** — no confirmation dialog for a reversible action; the persona does not need hand-holding.
...
- **Clamping is enforced in the domain, not only the editor.** The state machine must reject an overlapping result regardless of caller, with the editor clamping for usability. A UI-only guarantee is one bug away from an overlapping billing record.
...
- **`CaptureOrigin` transitions**: Add produces `ManualEntry`; any edit moves a block to the `*Adjusted` variant of its existing origin, never rewriting origin. Already expressible — no model change.
...
- Every one of the five operations moves the affected block to the `*Adjusted` variant of its existing `CaptureOrigin`, and never changes whether its origin is live or manual.
