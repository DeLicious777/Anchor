...
>
> **Accepted is not ready to implement.** Three things gated that, none of them design questions. **One is now closed:** risk **R14** — a `seq` consumed by an append that did not durably complete — was fixed in the writer on 2026-08-02, so `seq` uniqueness is now an invariant the writer enforces rather than one ADR 0004 assumed. **Two remain:** the Timeline Editor (#14) owes Add/Move/Resize a surface *and* owes undo a decision that may reopen ADR 0006; and the compaction snapshot (#8) must carry block ids or reconstruction loses reach over pre-watermark history.
>
...
- Delete is **either undoable or confirmed** — never both unconfirmed and irreversible. Whichever #14 provides, one of the two is asserted; a build in which neither holds fails this criterion.
