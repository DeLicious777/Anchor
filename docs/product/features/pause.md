...
- **The `Pause` transition and its command are built as an enabling slice (#23, 2026-08-07).** It closes `active` with `UserDetermined`, pushes its `StackFrame`, and leaves `active` as `None`, with frame construction shared with `Interrupt` so the two cannot drift. Nothing calls the command yet, so this is domain/IPC behaviour rather than a shipped Pause surface.
- **The transition is exercised by replay and snapshot tests, not only live tests.** `log::reader` calls `apply` directly with no dry-run guard, so a Pause arm that mutates before it can fail would corrupt replay rather than return an error — the hazard the `ReturnPrevious` arm's comment already documents.
...
- **Nothing may be logged while paused.** `gap::resolve` already produces no transition with nothing active. The former heartbeat loop, however, checked `active` and appended under separate locks, so Pause could land between them and a stale heartbeat could still be written. #23 moved the active check and append under one state lock; `should_beat` remains the policy predicate, while the atomic command path now makes the guarantee true.
...
- **Implementation is blocked on `visual-redesign.md`'s inputs** for its *visual* surface only — the spacing scale, palette and font weights are not in this repository. **The transition, command and tests landed in #23 without that dependency;** the feature remains unshipped because its display and live hotkey are still absent.
