...
- **Timeline Editor** _(added 2026-07-28; previously hidden inside "reconstruction")_: the graphical, direct-manipulation surface. Listed separately because it is a **hard prerequisite** — a boundary cannot be judged against a neighbour you cannot see — and because folding it into "reconstruction" was concealing implementation-critical scope from Planning. Its design questions are settled in [`docs/product/features/timeline-editor.md`](features/timeline-editor.md), not in `ideas/adjustable-timeline-view.md`.

  *(Amended 2026-08-04. The justification previously read "move and resize are **meaningless** against the tabular History View." That was too strong, and the Timeline Editor's own design disproved it: its selection panel drives Move and Resize from numeric fields, which is a selected-row interaction. The accurate claim is that those operations are **unjudgeable** without seeing the neighbour you are clamping against — which is what makes a spatial surface a prerequisite, and which the numeric fallback does not weaken. Note this doc's scope also now includes a shared **view range** across the timeline and History View, introduced by that feature's decision 2.)*
- **Pause, and `Start` as a first-class action** _(added 2026-07-28)_: Pause stops tracking without unwinding an open interruption stack — a specialised Interrupt that pushes a frame without starting a successor, so the paused task keeps its return path. A hole-closing change, not a convenience feature: `Complete` is currently rejected while the stack is non-empty. Requires `ReturnPrevious`/`ReturnOriginal` to become legal with no active task, and `Start` to be exposed as its own command (the transition exists; no command does). Continue Session is a UI action, not a transition. *(**Both prerequisites were already met when checked on 2026-08-07, and this sentence is retracted.** Neither return arm has ever required an active task: `ReturnPrevious` pops its frame and then calls `close_active_if_any`, which is a no-op when nothing is active, and `ReturnOriginal` checks only that the stack is non-empty — both then set `active` unconditionally. And a `start` command exists at `commands.rs:179`, registered in `lib.rs:142` and called by the frontend's `startTask`; it was added by the ADR 0005 command split and this line was never updated. **Pause therefore needs one transition, one command and a UI — not the three-part change described here.** Found while designing [`pause.md`](features/pause.md); the Continue Session clause is correct and stands. A scope estimate that was wrong in the pessimistic direction is still an **R11** instance: nobody checked.)*
