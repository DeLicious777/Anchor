...
**Path 1 remains open and is a prerequisite of implementing this ADR.** Tracked as risk **R14**. `seq` uniqueness is already required by ADR 0004's watermark filtering — two lines sharing a `seq` make that filter undefined regardless of identity — so this belongs in the writer, where the invariant lives, and must not be compensated for in the derivation. See Options Considered, G.
