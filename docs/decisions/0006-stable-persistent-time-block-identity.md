...
> **Amendment (2026-08-02): the open tension below is closed, and this ADR's Decision is unchanged.**
>
> A dedicated undo design pass and its independent review settled it. **Neither horn was taken, because neither was necessary: undo never re-creates a Time Block.** For MVP, Delete is **confirmed and has no undo** — a mistaken delete is recovered by re-adding, which honestly costs the block a new id and `ManualEntry` origin. The recorded future evolution, if confirmation proves insufficient, is **deferred commit**: the Delete transition is not appended until a boundary, so undo cancels an edit *before* it becomes history rather than reversing history. Under either, nothing is restored, so no id is ever quoted and the derivation above stays universal.
>
> **A third design was worked out in full and deliberately not adopted:** Delete as a *tombstone* with a `Restore` transition. It preserves this ADR intact — `Restore` creates zero Time Blocks, so the one-block-per-transition invariant holds, replay stays a pure fold, and identity is never destroyed. It was rejected not on architecture but on requirement: it buys durable, restart-surviving undo that no accepted document asks for, and charges for it in export, full-fidelity JSON, Capture Rate, the compaction snapshot payload, the reconstruction domain, and `next_default_name`. **This ADR is therefore not superseded and needs no successor.** The decision lives in [`timeline-reconstruction.md`](../product/features/timeline-reconstruction.md)'s UX section, because it constrains nothing outside the feature that makes it — the test in [`.claude/docs-standards.md`](../../.claude/docs-standards.md).
>
> Worth recording for the next reader: the investigation reached an ADR 0007 by challenging the assumption that Delete must destroy the block, and then dissolved it by asking whether that assumption needed changing at all. Requiring a new ADR is a **cost** of a design, not evidence for it.

**One open tension, now closed — original text retained.** `timeline-reconstruction.md` makes undo a hard prerequisite for unconfirmed Delete. Undoing a Delete forces a choice this ADR does not make: either the undo transition re-creates the block carrying its **original** id — which breaks "identity derives from the creating transition's `seq`" as a universal rule and makes the scheme a hybrid — or it creates a block with a **new** id and must carry the deleted block's full prior state, which is the replacement-state shape option D was rejected for. Both are arguable; neither is free. **This must be settled when undo is designed (#14), and it may require revisiting this ADR.**
...
- **[ADR 0004](0004-transition-log-format-and-torn-write-scheme.md) remains the sole authority for the append-only log format** — record shape, checksum framing, torn-write detection, compaction and the watermark. **This ADR changes none of its decisions**, and adds no field to any record.
  ...
  **It does add one normative requirement to the snapshot, and an earlier draft wrongly claimed compaction was "unaffected."** Blocks below the watermark are never replayed — `log/reader.rs` skips those lines entirely — so their identity can come only from the snapshot. Therefore:
  ...
  This is the *same gap* ADR 0005 had to close as assumption **A10**: ADR 0004 specified compaction's mechanism and never its payload, and a decision was built on the unstated assumption. One ADR later, in the same document, in the same shape. It is [`principles.md`](../principles.md) #4 verbatim — *only materialised state survives* — and it costs nothing to state now, because compaction was unimplemented when this was written (it shipped 2026-08-02, #8). Discovering it after a snapshot format ships costs a migration.
...
- ~~undo of a Delete is designed (#14) and requires re-creating a block under its original id;~~ **Resolved 2026-08-02 — this trigger did not fire.** See the amendment below. Retained struck through: the trigger was correctly identified, and the resolution is that the premise turned out to be avoidable, not that the concern was wrong.
