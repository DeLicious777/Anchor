...
>
> 1. ~~**The "stable on-disk contract" below is broken, deliberately and once.**~~ **Retracted 2026-07-29 — this was wrong, and this ADR's contract is fully intact.** ADR 0005 does split `completion_reason` into three fields, but that metadata **never reached the log**. `TransitionRecord` carries `seq`, `timestamp`, and a `TransitionPayload` and nothing else; `TimeBlock` appears nowhere in the `log` module — exactly as the Consequences below state ("resolved state … is derived by the state machine itself, not duplicated into the log"). The split is pure derived state, recomputed on every replay, so no format changed, no shim was needed, and no existing line became unreadable. Kept struck through rather than deleted: the error is the point, and it is why `docs/principles.md` #8 exists. Note the *snapshot* would have been affected, since it does serialise resolved state — but compaction is unimplemented, so there is nothing to migrate (#8).
...
>
> **Pointer, not an amendment — [ADR 0006](0006-stable-persistent-time-block-identity.md) (2026-07-29):** persistent Time Block identity is derived as `UUIDv5(namespace, seq)` from the `seq` this ADR defines. **Nothing in this ADR changes** — no record shape, no checksum framing, no torn-write handling, no compaction behaviour. It is recorded here only so a reader knows that `seq` acquired a second consumer, and that the *stability* of `seq` for the lifetime of a block is now load-bearing beyond watermark filtering.
