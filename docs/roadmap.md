...
**Planning is still not next** — see below. Four items entered MVP scope with no feature docs, and ADR 0005 carries nine open implementation-level items that must close before code.
...
4. **Planning workflow** (`.claude/workflows/planning.md`) — only after all of the above. It inherits the deferred MVP scope trade and must actually draw the v1.0/v1.1 line, or the scope grew with nothing given back.
