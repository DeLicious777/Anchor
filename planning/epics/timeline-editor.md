...
**The shared-range backend is built; no timeline surface exists.** #25 landed the persisted selection-intent store and get/set commands on 2026-08-07. Nothing calls them yet. `moveBlock` and `addBlock` exist in `app/src/lib/api.ts` with no caller; `resizeBlock` gained one on 2026-08-06 — the History View's numeric correction, which closed **R9**.

## Remaining work

1. ~~**The shared view range — store, persistence and get/set commands only.** Backend-owned, following the `paths.rs` one-file-per-concern pattern.~~ **Done in #25.** It persists selection intent: `today`, `this-week`, or `custom` with inclusive start/end calendar dates. Presets remain rolling after restart rather than freezing into resolved dates. The slice ships no user-reachable behaviour.
...
- **#25 — shared view range store and get/set commands. Implemented 2026-08-07;** item 1 above. It satisfies persistence and export-independence at the backend but no user-facing exit criterion. **The range control and History View filtering (item 1a) are explicitly not in it** and land with items 2–4.
...
**No — not any longer.** The only asset-independent item landed in #25; all remaining work is surface work tied to the design-system assets.
