...
- **No implementation code** until the Definition of Ready in `docs/README.md` / `.Codex/docs-standards.md` is fully met.
...
- Every non-trivial decision (Vision, Concept, ADR, feature doc) should survive a `grill-with-docs` pass (`.Codex/skills/grill-with-docs/`) before being marked `accepted`. This also keeps `docs/glossary.md` and `docs/decisions/` current as a side effect — don't skip it as "extra work."
...
Full detail in `.Codex/docs-standards.md` (expanded in Phase 4). Key points:
...
`/discovery-session`, `/new-feature <name>`, `/new-adr <title>`, `/new-epic <name>` (see `.Codex/commands/`) — use these to drive the phase workflows rather than freehanding equivalent work.
...
- Don't create GitHub issues, a GitHub Project, or planning artifacts in `planning/` before the Planning workflow's entry criteria are met (see `.Codex/workflows/planning.md`).
